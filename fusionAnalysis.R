...
cols <- c("Sample.ID", "case_id", "patient_id", "left_gene", "right_gene", "left_chr", "left_position", "right_chr", "right_position",
          "sample_id", "tool", "spanreadcount", "type", "var_level","ACMG_v1","ACMG_v2" ,"Brain_Cancer","CGCensus_Hereditary","Colon_Cancer",
...
makResultMatrix <- function(x) {
  ...
  data <- tryCatch({
    ...
    read.csv(paste0("./fusions.annotated/",x),sep="\t", header = FALSE, stringsAsFactors = FALSE )
...
### List all the files
fusionFiles <- list.files("./fusions.annotated/")[-1]

### concatenate files
finalResultMatrix <- rbindlist(lapply(fusionFiles, makResultMatrix), fill = TRUE)[,-c(33)]
